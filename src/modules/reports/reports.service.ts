...
export class ReportsService {
    ...
    async getReports(query) {
        if (query.filter && query.filter.owner) {
          const results = await Promise.allSettled([this.usersService.getUser({ filter: { nickname: query.filter.owner } }),
            this.teamsService.getTeam({ filter: { name: query.filter.owner } })])
      ...
      async getReport(reportOwner, reportName) {
        ...
        if (reports.length === 0) throw new NotFoundError({ message: "The specified report couldn't be found" })
        return reports[0]
      }
    
      async createReport(user, data, teamName) {
        if (!Validators.isValidReportName(data.src.name)) 
            throw new InvalidInputError({ message: `Study name can only consist of letters, numbers, '_' and '-'.` })
        ...
        let report = { _p_user: QueryParser.createForeignKey("_User", user.objectId) } as any 
        const usedNameQuery = {
          filter: {
            name: reportName,
            limit: 1
        ...
        if (teamName) {
          const { id: teamId } = await this.teamsService.getTeam({ filter: { name: teamName } })
          if (!await this.teamsService.hasPermissionLevel(user.objectId, teamName, "editor")) {
            throw new ForbiddenError({ message: "You are not allowed to create a report for this team" })
          }
        ...
        if (reports.length !== 0) throw new AlreadyExistsError({ message: "The specified name is already used by another report" })
        ...
        CREATE_REPORT_FIELDS.forEach(key => { if (metadata[key]) report[key] = metadata[key] })
        report = {
          ...report,
          name: reportName,
          provider: {
      ...
      async updateReport(userId, reportOwner, reportName, data) {
        ...
        if ((report.owner.type === "user" && report.owner.id !== userId) ||
          (report.owner.type === "team" && !await this.teamsService.hasPermissionLevel(userId, reportOwner, "editor"))) {
          throw new ForbiddenError({ message: "You are not allowed to delete this report" })
        }
      ...
      async pinReport(userId, reportOwner, reportName) {
        ...
        if ((report.owner.type === "user" && report.owner.id !== userId) ||
          (report.owner.type === "team" && !await this.teamsService.hasPermissionLevel(userId, reportOwner, "editor"))) {
          throw new ForbiddenError({ message: "You are not allowed to edit this report" })
        }
      ...
      async getBranches(reportOwner, reportName) {
        ...
        if (source.provider === LOCAL_REPORT_HOST) {
          ...
          const { accessToken } = await this.usersService.getUser({ filter: { _id: _p_user } })
          ...
          branches.forEach(branch => { branch.is_default = (branch === source.defaultBranch) })
        }
      ...
      async getCommits(reportOwner, reportName, branch) {
        ...
        if (source.provider === LOCAL_REPORT_HOST) throw new InvalidInputError({ message: "This functionality is not available in S3" })
