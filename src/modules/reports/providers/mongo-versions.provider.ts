...
export class VersionsMongoProvider extends MongoProvider {
    ...
    async getReportVersions(reportId) {
      const versions  = await this.read({ filter: { _p_study: QueryParser.createForeignKey("Study", reportId) } })
      return versions
    }
    ...
    async getReportVersion(reportId, version) {
      ...
      if (versions.length === 0) throw new NotFoundError({ message: "The specified version couldn't be found" })
      return versions[0]
    }
}
