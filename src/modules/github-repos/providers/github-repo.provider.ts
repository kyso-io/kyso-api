...
export class GithubReposProvider {
    ...
    async login(accessToken) {
      ...
      async searchRepos(filter, page, perPage) {
        ...
        if (orgs) {
          orgs.forEach(org => { q += `+org:${org.login}` })
        }
      ...
      async getFileHash(filePath, owner, repo, branch) {
        ...
        const filterData = (obj) => ({ type: obj.type, path: obj.path, hash: obj.sha, htmlUrl: obj.html_url })
      ...
      async getFileContent(fileSha, owner, repo) {
        try {
          ...
          if (err.status === 404) {
            throw new NotFoundError({ message: "The resource you are trying to access can't be found or isn't a file." })
          }
          throw err
        }
      }
      ...
      async getOrganizations() {
        const res = await this.octokit.orgs.listForAuthenticatedUser({ per_page: MAX_ORGANIZATIONS_PER_USER })
