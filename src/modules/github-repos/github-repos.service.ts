...
export class GithubReposService {
  ...
  _assignReports(user) {
    return async (repo) => {
      const reports = await this.reportsService.getReports({ filter: {
  ...
  async getRepos({ user, filter, page = 1, perPage = DEFAULT_REPOS_PER_PAGE }) {
    const repos = filter ? await this.provider.searchRepos(filter, page, perPage)
  ...
  async getRepo(user, owner, name) {
    ...
    if (!repo) throw new NotFoundError({ message: "The specified repository couldn't be found" })
