...
export class CommentsService {
    ...
    async getReportComments(reportId) {
        const comments = await this.provider.read({
          filter: { _p_study: QueryParser.createForeignKey("Study", reportId) },
          sort: { _created_at: -1 }
        })
      ...
      async getCommentWithChildren(commentId) {
        ...
        if (comments.length === 0) throw new NotFoundError({ message: "The specified comment couldn't be found" })
