...
export abstract class MongoProvider<T> {
  ...
  public async create(obj: any): Promise<any> {
    ...
    if (obj.id) {
      // The object comes with a predefined object id value, we must preserve it updating
      // the autogenerated one to the value passed
      obj._id = this.toObjectId(obj.id);
    }
    await this.getCollection().insertOne(obj);

    // The object comes without a predefined object id, update the undefined value with
    // the autogenerated one (mongo generates automatically an ID)
